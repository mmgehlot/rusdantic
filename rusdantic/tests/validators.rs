...
fn check_no_profanity(value: &str) -> Result<(), ValidationError> {
...
fn check_not_banned(value: &str, ctx: &dyn std::any::Any) -> Result<(), ValidationError> {
    // Downcast to our expected context type
    if let Some(app_ctx) = ctx.downcast_ref::<AppContext>() {
        if app_ctx.banned_words.iter().any(|w| w == value) {
            return Err(ValidationError::new(
                "banned",
