...
fn expand_enum(parsed: RusdanticInput) -> syn::Result<TokenStream> {
    ...
    let _serde_repr_attrs = if parsed.untagged {
    ...
    let _rename_all_attr = parsed.rename_all.as_ref().map(|r| {
        quote::quote! { #[serde(rename_all = #r)] }
    });

    // Build variant serde rename attributes
    let _variant_rename_attrs: Vec<TokenStream> = variants
        .iter()
        .map(|v| {
...
fn build_ir(parsed: RusdanticInput) -> syn::Result<ValidatedStruct> {
    let rename_all = parsed.rename_all.as_deref().and_then(RenameAll::from_str);
