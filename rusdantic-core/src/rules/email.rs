...
pub const EMAIL_REGEX_PATTERN: &str = r"(?i)^[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*\.[a-z]{2,}$";
...
pub fn validate_email<T: AsStr>(value: &T, path: &[PathSegment], errors: &mut ValidationErrors) {
    let s = value.as_str_ref();

    // Quick checks before regex (performance optimization)
    if s.is_empty() || !s.contains('@') || s.len() > 254 {
        errors.add(ValidationError::new("email", "invalid email format").with_path(path.to_vec()));
        return;
    }
